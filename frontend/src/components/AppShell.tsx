...
export function AppShell() {
  ...
  const dashboard = user ? ({ student:'/student', requester:'/requester', reviewer:'/reviewer', admin:'/admin' } as const)[user.role] : null
  return <div className="site-shell">
    <header className="site-header">
      ...
      <nav className={open ? 'nav open' : 'nav'} aria-label="主导航">
        ...
        {user ? <><NavLink to={dashboard!}>我的工作台</NavLink><span className="nav-user">档案号 · {user.username}</span><button className="text-button" onClick={logout}><LogOut size={15}/>退出</button></> : <><NavLink to="/login">登录</NavLink><Link className="nav-cta" to="/register">建立档案</Link></>}
      </nav>
    </header>
    <main><Outlet /></main>
